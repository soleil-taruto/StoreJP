...
namespace Charlotte
{
	class Program
	{
		...
		private void Main3()
		{
			...
			Main4(new ArgsReader(new string[] { @"C:\home\GitHub\StoreJ" }));
		...
		private void Main5(ArgsReader ar)
		{
			...
			string[] paths = SCommon.Concat(repositoryDirs.Select(repositoryDir => GetCommitmentPaths(repositoryDir).ToArray()).ToArray()).ToArray();
			...
			foreach (string path in paths) // ファイルの追加・削除
			{
				if (IsEmptyDir(path))
				{
					string markFile = Path.Combine(path, Consts.EMPTY_DIR_MARK_FILE_NAME);
			...
			foreach (string path in paths) // ファイル・ディレクトリ名の変更
			{
				...
				string escLocalName = EscapeLocalName(localName);
		...
		private string EscapeLocalName(string localName)
		{
