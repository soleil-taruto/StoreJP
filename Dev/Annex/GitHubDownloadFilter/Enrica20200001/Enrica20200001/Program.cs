...
namespace Charlotte
{
	class Program
	{
		...
		private void Main3()
		{
			...
			Main4(new ArgsReader(new string[] { @"C:\temp\StoreJ-main" }));
		...
		private void FilterMain(string rootDir)
		{
			...
			for (int index = 0; index < files.Length; index++) // ファイルの削除
			{
				...
				if (SCommon.EqualsIgnoreCase(Path.GetFileName(file), Consts.EMPTY_DIR_MARK_FILE_NAME))
				{
			...
			foreach (string file in files) // ファイル名の変更
			{
				...
				if (!SCommon.EqualsIgnoreCase(localName, trueLocalName))
				{
